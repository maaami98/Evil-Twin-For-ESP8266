...
class HTTPServer(Server):
    def __init__(self, poller, local_ip):
        ...
        self.request = dict()
        self.conns = dict()
        self.routes = {b'/exy':b'exy',b'/login':self.login,b'/ssid':self.get_ssid}
        self.password=None
        self.ssid=None
        # queue up to 5 connection requests before refusing
        self.sock.listen(5)
        self.sock.setblocking(False)
    def set_route(self,url,file):
        self.routes.update( {url:file} )
    ...
    def get_ssid(self, params):
        headers = b"HTTP/1.1 200 OK\r\n"
        body = self.ssid.decode()
        return body, headers
